<?php
session_start();
require_once '../../Database/Connection.php';
if(!isset($_SESSION['id_transaksi'])){
    header("location:Transaksi.php");
}
$id = $_SESSION['id_transaksi'];
$transaksiQuery = mysqli_query($conn, "SELECT *FROM transaksi WHERE id_transaksi = $id");
$transaksi = mysqli_fetch_assoc($transaksiQuery);
?>
<!DOCTYPE html>
<html>
<head>
    <title>Home</title>
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css">
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js">
    </script>
    <link rel="stylesheet" href="../Sidebar.css">
    <link rel="stylesheet" href="TableTransaksi.css">
</head>
<body>
<div class="wrapper">
    <nav id="sidebar">
        <div class="user-panel">
            <div class="info-image-sidebar">
                <img src=data:image/png;base64,>
            </div>
            <div class="info-user-sidebar">
                <span>Krisna Pranata</span>
            </div>
        </div>
        <ul class="list-unstyled components">
            <li class="header">
                MAIN NAVIGATION
            </li>
            <li>
                <a href="../Dashboard/HomePegawai.php?page=1&data=10">
                    <i class="glyphicon glyphicon-home"></i>
                    Dashboard
                </a>
            </li>
            <li>
                <a href="../Transaksi/Transaksi.php" class="activeSideBar" style="border-left-color: #3c8dbc;">
                    <i class="glyphicon glyphicon-briefcase"></i>
                    Transaksi
                </a>
            </li>
            <li>
                <a href="../Member/Member.php?page=1&data=10">
                    <i class="glyphicon glyphicon-user"></i>
                    Member
                </a>
            </li>
            <li>
                <a href="../Pegawai/ProfilePegawai.php">
                    <i class="glyphicon glyphicon-info-sign"></i>
                    Pegawai
                </a>
            </li>
            <li>
                <a href="../Logout.php">
                    <i class="glyphicon glyphicon-off"></i>
                    Log Out
                </a>
            </li>
        </ul>
    </nav>
    <div class="main-navbar">
        <div class="main-header">
            <a href="#" class="logo">
                <div class="logo-lg" id="logo">
                    <b>Laundry</b>
                    <strong>LA</strong>
                </div>
            </a>
        </div>
        <nav class="navbar" id="navbar">
            <a href="#" class="navbar-left" id="sidebarCollapse">
                <i class="glyphicon glyphicon-th-list">
                </i>
            </a>

        </nav>
    </div>
    <div class="mainContent" id="mainContent">
        <div class="contentHeader content" id="contentHeader" style="margin-bottom: 40px;">
            <h3 style="text-align: center;"><b>PEMBAYARAN</b></h3>
            <form action="FunctionTransaksi.php" method="post">
                <table style="margin-top: 40px">
                    <tr>
                        <td style="width: 20%">
                            <label style="float: left" class="control-label">ID Transaksi</label>
                        </td>
                        <td style="width: 80%">
                            <input type="text" class="form-control" placeholder="Masukan ID Transaksi" name="id_transaksi" value="<?php echo $_SESSION['id_transaksi']?>">
                        </td>
                    </tr>
                    <tr>
                        <td style="width: 20%">
                            <label style="float: left" class="control-label">Total Harga</label>
                        </td>
                        <td style="width: 80%">
                            <input type="text" class="form-control" name="total_harga" readonly value="<?php echo $transaksi['total_harga']?>" id="harga">
                        </td>
                    </tr>
                    <tr>
                        <td style="width: 20%">
                            <label style="float: left" class="control-label">Biaya</label>
                        </td>
                        <td style="width: 80%">
                            <input type="text" class="form-control" name="biaya" id="biaya" onkeyup="myFunction()">
                        </td>
                    </tr>
                    <tr>
                        <td style="width: 20%">
                            <label style="float: left" class="control-label">Kembalian</label>
                        </td>
                        <td style="width: 80%">
                            <input type="text" class="form-control" name="kembalian" id="kembalian">
                        </td>
                    </tr>
                   </table>
                <div style="text-align: center">
                    <button type="submit" class="btn btn-primary" style="margin-top: 30px;width: 10%;height: 40px" name="update">
                        <b>UPDATE</b>
                    </button>
                    <button type="submit" class="btn btn-warning" style="margin-top: 30px;width: 10%;height: 40px" name="cancel">
                        <b>CANCEL</b>
                    </button>
                    <button type="submit" class="btn btn-danger" style="margin-top: 30px;width: 10%;height: 40px" name="batal">
                        <b>BATAL</b>
                    </button>
                </div>
            </form>
        </div>
    </div>
</body>
<script>
    function myFunction() {
        var biaya = document.getElementById("biaya").value;
        var harga = document.getElementById("harga").value;
        var kembalian = biaya - harga;
        document.getElementById("kembalian").value = kembalian;
    }
</script>
</html>
